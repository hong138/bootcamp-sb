...
class JPHControllerTest {
  ...
  @MockBean // because main code using Autowired for bean object
  private JPHService jphService;
  // Mock, MockBean, Autowired
  ...
  void testJPHGetAllUsers() throws Exception {
    ...
    // Define hoe the Result is being mocked
    ...
    mockMvc.perform(get("/jph/users"))
        ...
        .andExpect(jsonPath("$[*].website").value(hasItem("vincentlau@gmail.com")))
        .andExpect(jsonPath("$[*].username").value(hasItem("jennyshe")))
        .andExpect(jsonPath("$[*].website").value(hasItem("jennyshe@gmail.com")));

    verify(jphService, times(2)).getUsers();

    // ! Approach 3
    String json = mockMvc.perform(get("/jph/users"))
                  .andReturn()
                  .getResponse()
                  .getContentAsString();

    // from Json to Java object (De-serialize)
    UserDTO[] userDTOs = new ObjectMapper().readValue(json, UserDTO[].class);
